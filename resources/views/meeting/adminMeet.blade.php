        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($requests as $req)
                <div class="bg-white shadow-sm rounded-lg p-4 flex flex-col justify-between">
                    ...
                    {{-- Info Meeting --}}
                    <div>
                        <h3 class="font-bold text-lg mb-2">{{ $req->title }}</h3>
                    ...
                    <div class="mt-4 flex gap-2">
                        <form method="POST" action="{{ route('admin.meetings.approve', $req->id) }}" class="flex-1">
                            @csrf
                            <button type="submit" class="w-full bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 text-sm">
                                Accept
                            </button>
                        </form>
                        ...
                        <form method="POST" action="{{ route('admin.meetings.reject', $req->id) }}" class="flex-1">
                            @csrf
                            <button type="submit" class="w-full bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700 text-sm">
                                Reject
                            </button>
                        </form>
                    </div>
            ...
            @empty
                ...
                <div class="bg-white shadow-sm rounded-lg p-6 col-span-full text-center text-gray-500">
                    <div class="h-32 bg-gray-100 flex items-center justify-center rounded mb-4">
                        <span class="text-gray-400 text-sm">No meeting requests yet</span>
                    </div>
                    {{-- <a href="{{ route('meetings.create') }}"  --}}
                       class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
