<x-app-layout>
    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="p-6">
                    @forelse($pending as $rev)
                        <div class="flex items-start justify-between p-4 border border-gray-100 rounded-lg mb-4">
                            <div class="flex-1 pr-4">
                                <p class="font-bold text-gray-900">{{ $rev->user->name ?? 'User' }}</p>
                            ...
                            <div class="flex gap-2">
                                <form method="POST" action="{{ route('admin.shopReviews.approve', $rev->id) }}">
                                    @csrf
                                    <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">Approve</button>
                                </form>
                                <form method="POST" action="{{ route('admin.shopReviews.reject', $rev->id) }}">
                                    @csrf
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                    <h3 class="text-lg font-bold text-gray-900">Approved Reviews</h3>
                </div>
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @forelse($approved as $rev)
                        <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                            <div class="flex items-center mb-2">
                                <p class="font-bold mr-2 text-gray-900">{{ $rev->user->name ?? 'User' }}</p>
                                <p class="text-yellow-600 font-semibold">⭐ {{ $rev->rating }}/5</p>
                            </div>
                            @if($rev->comment)
                    ...
                    @empty
                        <p class="text-gray-500">No approved reviews yet.</p>
                    @endforelse
                </div>
            </div>
